...
/*   Updated: 2022/09/03 04:22:18 by hhaddouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_player(int keycode, t_vars *game)
{
	...
	if (keycode == 13 && (game->map_data.two_dmap[y - 1][x] != '1'))
		move_up(&x, &y, game->map_data.two_dmap, game);
	else if ((keycode == 1) && (game->map_data.two_dmap[y + 1][x] != '1'))
		move_down(&x, &y, game->map_data.two_dmap, game);
	else if ((keycode == 2) && (game->map_data.two_dmap[y][x + 1] != '1'))
		move_left(&x, &y, game->map_data.two_dmap, game);
	else if ((keycode == 0) && (game->map_data.two_dmap[y][x - 1] != '1'))
		move_right(&x, &y, game->map_data.two_dmap, game);
	else if (keycode == 53)
		close_win(game);
